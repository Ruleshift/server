...
func (s *ControlStore) GetModule(ctx context.Context, d, m string) (controlplane.Module, error) {
	...
	err := s.platform.control.QueryRowContext(ctx, `SELECT developer_id,module_key,display_name,active_version,created_at,updated_at FROM modules WHERE developer_id=$1 AND module_key=$2`, d, m).Scan(&value.DeveloperID, &value.Key, &value.DisplayName, &active, &value.CreatedAt, &value.UpdatedAt)
	...
	value.ActiveVersion = active.String
	return value, nil
}
...
func (s *ControlStore) SetValidation(ctx context.Context, value controlplane.ValidationRun) error {
	...
	_, err := s.platform.control.ExecContext(ctx, `INSERT INTO module_validation_runs(developer_id,module_id,version,started_at,finished_at,result,logs) VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT(developer_id,module_id,version) DO UPDATE SET started_at=EXCLUDED.started_at,finished_at=EXCLUDED.finished_at,result=EXCLUDED.result,logs=EXCLUDED.logs`, value.DeveloperID, value.ModuleID, value.Version, value.StartedAt, value.FinishedAt, string(value.Result), value.Logs)
	return err
}
func (s *ControlStore) GetValidation(ctx context.Context, d, m, v string) (controlplane.ValidationRun, error) {
	...
	var status string
	err := s.platform.control.QueryRowContext(ctx, `SELECT started_at,finished_at,result,logs FROM module_validation_runs WHERE developer_id=$1 AND module_id=$2 AND version=$3`, d, m, v).Scan(&value.StartedAt, &value.FinishedAt, &status, &value.Logs)
	if errors.Is(err, sql.ErrNoRows) {
...
func (s *ControlStore) Activate(ctx context.Context, d, m, v string) error {
	tx, err := s.platform.control.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
	defer tx.Rollback()
	if _, err = tx.ExecContext(ctx, `UPDATE module_versions SET lifecycle_status='inactive',updated_at=NOW() WHERE developer_id=$1 AND module_id=$2 AND lifecycle_status='active'`, d, m); err != nil {
		return err
	}
	result, err := tx.ExecContext(ctx, `UPDATE module_versions SET lifecycle_status='active',updated_at=NOW() WHERE developer_id=$1 AND module_id=$2 AND version=$3 AND lifecycle_status='validating'`, d, m, v)
	if err != nil {
		return err
	}
	rows, _ := result.RowsAffected()
	...
	if _, err = tx.ExecContext(ctx, `UPDATE modules SET active_version=$3,updated_at=NOW() WHERE developer_id=$1 AND module_key=$2`, d, m, v); err != nil {
		return err
	}
	return tx.Commit()
}
func (s *ControlStore) MarkStatus(ctx context.Context, d, m, v string, status controlplane.VersionStatus) error {
	result, err := s.platform.control.ExecContext(ctx, `UPDATE module_versions SET lifecycle_status=$4,updated_at=NOW() WHERE developer_id=$1 AND module_id=$2 AND version=$3`, d, m, v, string(status))
	if err != nil {
		return err
	}
	rows, _ := result.RowsAffected()
...
func (s *ControlStore) ResolveForNewRoom(ctx context.Context, d, m, v string) (controlplane.Version, error) {
	...
	var active sql.NullString
	if err := s.platform.control.QueryRowContext(ctx, `SELECT active_version FROM modules WHERE developer_id=$1 AND module_key=$2`, d, m).Scan(&active); errors.Is(err, sql.ErrNoRows) {
		return controlplane.Version{}, controlplane.ErrModuleNotFound
	} else if err != nil {
		return controlplane.Version{}, err
	}
	if active.Valid {
		value, err := s.GetVersion(ctx, d, m, active.String)
		if err == nil && value.Status == controlplane.StatusActive {
			return value, nil
		}
	}
	var fallback string
	err := s.platform.control.QueryRowContext(ctx, `SELECT version FROM module_versions WHERE developer_id=$1 AND module_id=$2 AND lifecycle_status='inactive' ORDER BY updated_at DESC LIMIT 1`, d, m).Scan(&fallback)
	if errors.Is(err, sql.ErrNoRows) {
		return controlplane.Version{}, controlplane.ErrVersionNotFound
	}
	if err != nil {
